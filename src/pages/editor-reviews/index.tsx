...
import { getEditorReviews, deleteEditorReviewById } from 'apiSdk/editor-reviews';
import { EditorReviewInterface } from 'interfaces/editor-review';

type ColumnType = ColumnDef<EditorReviewInterface, unknown>;

interface EditorReviewListPageProps {
...
export function EditorReviewListPage(props: EditorReviewListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EditorReviewInterface>>(
    () => `/editor-reviews?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteEditorReviewById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: EditorReviewInterface) => {
    if (hasAccess('editor_review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/editor-reviews/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/editor-reviews/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('editor_review', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/editor-reviews/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('editor_review', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="editor_review" />
        </Flex>

        {hasAccess('editor_review', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/editor-reviews/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
