...
import { getSubscriberContentById, updateSubscriberContentById } from 'apiSdk/subscriber-contents';
...
function SubscriberContentEditPage() {
  ...
  const handleSubmit = async (values: SubscriberContentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSubscriberContentById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Subscriber Content',
              isCurrent: true,
            },
          ]}
        />
